...
auto init_lexer(std::string source) -> std::shared_ptr<Lexer> {
...
auto lexer_advance_current_token(std::shared_ptr<Lexer> lex, Token_Type type) -> std::shared_ptr<Syntax_Token> {
    std::string value;
    value += lex->_c;
    std::shared_ptr<Syntax_Token> tok = init_token(value, type);
...
auto lexer_parse_identifier(std::shared_ptr<Lexer> lex) -> std::shared_ptr<Syntax_Token> {
    ...
    return init_token(identifier, Token_Type::identifier_token);
}
...
auto lexer_parse_number(std::shared_ptr<Lexer> lex) -> std::shared_ptr<Syntax_Token> {
    ...
    return init_token(number, Token_Type::number_token);
}
        ...
        switch(lex->_c) {
            case '=': {
                ...
                if(lexer_peek(lex, 1) == '>') {
                    ...
                    return lexer_return_first_token(lex, init_token("=>", Token_Type::arrow_right_return_type_token));
                }
                //Equal equal to
                else if(lexer_peek(lex, 1) == '=') {
                    lexer_advance(lex);

                    return lexer_return_first_token(lex, init_token("==", Token_Type::equal_equal_to_token));
                }
            ...
            case '<': {
                ...
                if(lexer_peek(lex, 1) == '=') {
                    ...
                    return lexer_return_first_token(lex, init_token("<=", Token_Type::lesser_than_or_equal_to_token));
                }
            ...
            case '>': {
                ...
                if(lexer_peek(lex, 1) == '=') {
                    ...
                    return lexer_return_first_token(lex, init_token(">=", Token_Type::greater_than_or_equal_to_token));
                }
            ...
            default: {
                ...
                return init_token("", Token_Type::bad_token);
            }
        }
    }

    return init_token("\0", Token_Type::eof_token);
}